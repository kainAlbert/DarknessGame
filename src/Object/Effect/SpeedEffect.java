...
public class SpeedEffect extends CharacterBase{
	...
	public void initialize( int id ){
		...
		super.initialize(
				...
				id,
				new Vector2(),
				DefineEffect.SPEED_SIZE,
				DefineEffect.SPEED_RESIZE,
				0);
	...
	private void changePosSize(){
		...
		// サイズ設定
		mSize = new Vector2( DefineEffect.SPEED_SIZE.x, DefineEffect.SPEED_SIZE.y );

		if( player.getSpeedTimer() <= 0 ) mSize = new Vector2();
	...
	private void changeReSize(){
		...
		if( mTimer < DefineEffect.SPEED_ANIM_TIMER ) return;
		...
		// リサイズが最大になったら最初に戻す
		if( mReSize.x > mReSizeDistance.x * DefineEffect.SPEED_IMAGE_NUM ){
